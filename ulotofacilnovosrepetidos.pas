...
type
    ...
    TLotofacilNovosRepetidos = class(TThread)
        ...
        // Armazena a frequência de bolas.
        lotofacil_frequencia_bolas: array[0..25] of integer;

        frq_grp_1: array of integer;
        frq_grp_2: array of array of integer;
        frq_grp_3: array of array of array of integer;
        frq_grp_4: array of array of array of array of integer;
        frq_grp_5: array of array of array of array of array of integer;
    ...
    private
        ...
        function obter_frequencia_das_bolas(numero_do_concurso: integer): boolean;
        function obter_frequencia_por_grupos(numero_do_concurso: integer): boolean;
    ...
    public
        ...
        property OnStatusAtualizacao: TLotofacilNovosRepetidosStatusAtualizacao
            read fStatus_Atualizacao write fStatus_Atualizacao;
        property OnStatusConcluido: TLotofacilNovosRepetidosStatusConcluido
            read fStatus_Concluido write fStatus_Concluido;
...
begin
    ...
    if not obter_frequencia_por_grupos(fConcurso) then
    begin
        Exit;
    end;

    if not obter_frequencia_das_bolas(fConcurso) then
    begin
        Exit;
    end;

    //fStatus_Concluido('OK');
...
{
 Obtém a frequência por combinações de bolas, em grupos de 1 a 5 bolas, relativo a todas
 as combinações já sorteadas da lotofacil.
}
function TLotofacilNovosRepetidos.obter_frequencia_por_grupos(numero_do_concurso: integer): boolean;
const
    qt_cmb_grupo_1 = 25;
    qt_cmb_grupo_2 = 25 * 25;
    qt_cmb_grupo_3 = 25 * 25 * 25;
    qt_cmb_grupo_4 = 25 * 25 * 25 * 25;
    qt_cmb_grupo_5 = 25 * 25 * 25 * 25 * 25;
var
    sql_query:    TZQuery;
    bolas_do_concurso: array[1..15] of integer;
    bola_atual, uA, uB, uC, uD, uE, bola_b1, bola_b2, bola_b3, bola_b4, bola_b5: integer;
    qt_registros: longint;
begin
    SetLength(frq_grp_1, 26);
    SetLength(frq_grp_2, 26, 26);
    SetLength(frq_grp_3, 26, 26, 26);
    SetLength(frq_grp_4, 26, 26, 26, 26);
    SetLength(frq_grp_5, 26, 26, 26, 26, 26);

    // Verifica se os arranjos foram alocados corretamente.
    if High(frq_grp_1) = -1 then
    begin
        fStatus_Mensagem := 'Não foi possível alocar memória pra o arranjo de grupo com 1 bola.';
        Synchronize(@DoStatus);
        Exit(False);
    end;

    if High(frq_grp_2) = -1 then
    begin
        fStatus_Mensagem := 'Não foi possível alocar memória pra o arranjo de grupo com 2 bolas.';
        Synchronize(@DoStatus);
        Exit(False);
    end;

    if High(frq_grp_3) = -1 then
    begin
        fStatus_Mensagem := 'Não foi possível alocar memória pra o arranjo de grupo com 3 bolas.';
        Synchronize(@DoStatus);
        Exit(False);
    end;

    if High(frq_grp_4) = -1 then
    begin
        fStatus_Mensagem := 'Não foi possível alocar memória pra o arranjo de grupo com 4 bolas.';
        Synchronize(@DoStatus);
        Exit(False);
    end;

    if High(frq_grp_5) = -1 then
    begin
        fStatus_Mensagem := 'Não foi possível alocar memória pra o arranjo de grupo com 5 bolas.';
        Synchronize(@DoStatus);
        Exit(False);
    end;

    // Vamos obter todas as combinações já sorteadas e vamos realizar a estatística de tais combinações
    // em grupos de 1 a 5 bolas.
    try
        ...
        if qt_registros <= 0 then
        begin
            fStatus_Mensagem := 'Nenhum registro localizado.';
            Synchronize(@DoStatus);
        ...
        // Zera os arranjos antes de popular os dados.
        //FillByte(frq_grp_1[0], sizeof(integer) * 25, 0);
        //FillByte(frq_grp_2[0, 0], sizeof(integer) * 25 * 25, 0);
        //FillByte(frq_grp_3[0, 0, 0], sizeof(integer) * 25 * 25 * 25, 0);
        //FillByte(frq_grp_4[0, 0, 0, 0], sizeof(integer) * 25 * 25 * 25 * 25 * 25, 0);
        //FillByte(frq_grp_5[0, 0, 0, 0, 0], sizeof(integer) * 25 * 25 * 25 * 25 * 25 * 25, 0);
        ...
        begin
            ...
            begin
                ...
                frq_grp_1[bola_atual] := frq_grp_1[bola_atual] + 1;
            end;

            // Grupos de 2 bolas.
            for uA := 1 to 15 do
            begin
                bola_b1 := bolas_do_concurso[uA];
                for uB := uA + 1 to 15 do
                begin
                    bola_b2 := bolas_do_concurso[uB];
                    frq_grp_2[bola_b1, bola_b2] := frq_grp_2[bola_b1, bola_b2] + 1;
                end;
            end;
            ...
            begin
                ...
                begin
                    ...
                    begin
                        bola_b3 := bolas_do_concurso[uC];
                        frq_grp_3[bola_b1, bola_b2, bola_b3] :=
                            frq_grp_3[bola_b1, bola_b2, bola_b3] + 1;
                    end;
                end;
            end;
            ...
            begin
                ...
                begin
                    ...
                    begin
                        bola_b3 := bolas_do_concurso[uC];
                        for uD := uC + 1 to 15 do
                        begin
                            bola_b4 := bolas_do_concurso[uD];
                            frq_grp_4[bola_b1, bola_b2, bola_b3, bola_b4] :=
                                frq_grp_4[bola_b1, bola_b2, bola_b3, bola_b4] + 1;
            ...
            begin
                ...
                begin
                    ...
                    begin
                        ...
                        begin
                            bola_b4 := bolas_do_concurso[uD];
                            for uE := uD + 1 to 15 do
                            begin
                                bola_b5 := bolas_do_concurso[uE];
                                frq_grp_5[bola_b1, bola_b2, bola_b3, bola_b4, bola_b5] :=
                                    frq_grp_5[bola_b1, bola_b2, bola_b3, bola_b4, bola_b5] + 1;
...
procedure TLotofacilNovosRepetidos.gerar_frequencia_das_combinacoes;
const
    TOTAL_DE_COMBINACOES = 6874010;
var
    ltf_id, b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11, b12, b13, b14, b15, soma_grupo_1,
    soma_grupo_2, soma_grupo_3, soma_grupo_4, uA, uB, uC, soma_grupo_5, uD, uE, b16, b17, b18: integer;
    lotofacil_cmb: array[0..18] of byte;
    bola_atual, bola_b1, bola_b2, bola_b3, bola_b4, bola_b5: byte;
begin
    ltf_id := 0;

    // Há na lotofacil, 6874010 combinações, incluindo combinações de 15 a 18 bolas.
    SetLength(sm_frq_grupo, TOTAL_DE_COMBINACOES + 1, 6);
                            ...
                            begin
                                ...
                                begin
                                    ...
                                    begin
                                        ...
                                        begin
                                            lotofacil_cmb[10] := b10;
                                            for b11 := b10 + 1 to 25 do
                                            begin
                                                lotofacil_cmb[11] := b11;
                                                for b12 := b11 + 1 to 25 do
                                                begin
                                                    lotofacil_cmb[12] := b12;
                                                    for b13 := b12 + 1 to 25 do
                                                    begin
                                                        lotofacil_cmb[13] := b13;
                                                        for b14 := b13 + 1 to 25 do
                                                        begin
                                                            lotofacil_cmb[14] := b14;
                                                            for b15 := b14 + 1 to 25 do
                                                            begin
                                                                lotofacil_cmb[15] := b15;
                                                                begin
                                                                    // Soma dos grupos
                                                                    soma_grupo_1 := 0;
                                                                    for uA := 1 to 15 do
                                                                    ...
                                                                    Inc(ltf_id);
                                                                    sm_frq_grupo[ltf_id, 1] := soma_grupo_1;
                                                                    sm_frq_grupo[ltf_id, 2] := soma_grupo_2;
                                                                    sm_frq_grupo[ltf_id, 3] := soma_grupo_3;
                                                                    sm_frq_grupo[ltf_id, 4] := soma_grupo_4;
                                                                    sm_frq_grupo[ltf_id, 5] := soma_grupo_5;

                                                                    if ltf_id mod 500000 = 0 then
                                                                    begin
                                                                        fStatus_Mensagem :=
                                                                            'ltf_id: ' + IntToStr(ltf_id);
                                                                        Synchronize(@DoStatus);
                                                                    end;
                                                                end;
                                                            end;
                                                                    ...
                                                                    begin
                                                                        ...
                                                                        begin
                                                                            ...
                                                                            begin
                                                                                ...
                                                                                Inc(ltf_id);
                                                                                sm_frq_grupo[ltf_id, 1] :=
                                                                                    soma_grupo_1;
                                                                                sm_frq_grupo[ltf_id, 2] :=
                                                                                    soma_grupo_2;
                                                                                sm_frq_grupo[ltf_id, 3] :=
                                                                                    soma_grupo_3;
                                                                                sm_frq_grupo[ltf_id, 4] :=
                                                                                    soma_grupo_4;
                                                                                sm_frq_grupo[ltf_id, 5] :=
                                                                                    soma_grupo_5;
                                                                            end;
                                                                        end;
                                                                    end;
...
function TLotofacilNovosRepetidos.Obter_id_de_comparacao_de_bolas_na_mesma_coluna(
    qt_de_bolas_comuns, qt_de_bolas_subindo, qt_de_bolas_descendo: integer): integer;
begin
    fStatus_Mensagem_Erro := '';

    if not (qt_de_bolas_comuns in [0..15]) then
    begin
        fStatus_Mensagem_Erro := fStatus_Mensagem_Erro + #10#13 +
            'Campo: qt_de_bolas_comuns_b1_a_b15 fora do intervalo';
    end;

    if not (qt_de_bolas_subindo in [0..15]) then
    begin
        fStatus_Mensagem_Erro := fStatus_Mensagem_Erro + #10#13 +
            'Campo: qt_de_bolas_subindo_b1_a_b15 fora do intervalo';
    end;

    if not (qt_de_bolas_descendo in [0..15]) then
    begin
        fStatus_Mensagem_Erro := fStatus_Mensagem_Erro + #10#13 +
            'Campo: qt_de_bolas_descendo_b1_a_b15 fora do intervalo';
    end;

    if 15 <> (qt_de_bolas_comuns + qt_de_bolas_subindo + qt_de_bolas_descendo) then
    begin
        fStatus_Mensagem_Erro := fStatus_Mensagem_Erro + #10#13 + ' A soma dos campos: ' +
            QuotedStr('qt_de_bolas_comuns_b1_a_b15') + ', ' + QuotedStr('qt_de_bolas_subindo_b1_a_b15') +
...
var
    sql_query: TSQLQuery;
    qt_bolas_comuns_b1_a_b15, qt_bolas_subindo_b1_a_b15, qt_bolas_descendo_b1_a_b15, qt_registros: longint;
begin
    ...
    try
        ...
        begin
            ...
            if not ((qt_bolas_comuns_b1_a_b15 in [0..15]) and (qt_bolas_subindo_b1_a_b15 in [0..15]) and
                (qt_bolas_descendo_b1_a_b15 in [0..15])) then
            begin
                fStatus_Mensagem_Erro :=
                    'Erro, no registro: cmp_b_id: ' + IntToStr(
                    sql_query.FieldByName('cmp_b_id').AsInteger) + 'A soma dos valores dos campos ' +
                    QuotedStr('qt_bolas_comuns_b1_a_15') + ', ' + QuotedStr(
                    'qt_bolas_subindo_b1_a_15') + ', ' + QuotedStr('qt_bolas_descendo_b1_a_15') +
                    ' não é igual a 15';
                Exit(False);
            end;
            f_ltf_id_bolas_na_mesma_coluna[qt_bolas_comuns_b1_a_b15,
                qt_bolas_subindo_b1_a_b15, qt_bolas_descendo_b1_a_b15] :=
                byte(sql_query.FieldByName('cmp_b_id').AsInteger);
...
begin
    if Assigned(fStatus_Atualizacao) then
    begin
